...
extension SKScene {
    ...
    func createSlidingJoint(in parent: SKNode, constrainInside: SKNode? = nil) {
        /// link object
        ...
        if let rectangle = constrainInside {
            linkStart.constraints = [createConstraintsWithRectangle(node: linkStart, rectangle: rectangle)]
        }
        ...
        if let rectangle = constrainInside {
            linkEnd.constraints = [createConstraintsWithRectangle(node: linkEnd, rectangle: rectangle)]
        }
    ...
    func createFixedJointWithSpinningWheels(in parent: SKNode, constrainInside: SKNode? = nil) {
        ...
        wheel1.physicsBody?.friction = 1
        if let rectangle = constrainInside {
            wheel1.constraints = [createConstraintsWithRectangle(node: wheel1, rectangle: rectangle)]
        }
        parent.addChild(wheel1)
        ...
        wheel2.physicsBody?.friction = 1
        if let rectangle = constrainInside {
            wheel2.constraints = [createConstraintsWithRectangle(node: wheel2, rectangle: rectangle)]
        }
    ...
    func createPinJoint(parent: SKNode, as physicsCatgeory: PhysicsCategory? = nil) {
        ...
        if let category = physicsCatgeory {
            setupPhysicsCategories(node: objectA, as: category)
        }
        ...
        if let category = physicsCatgeory {
            setupPhysicsCategories(node: objectB, as: category)
        }
