...
public struct SimpleRotationInertia: ViewModifier {
                            ...
                            if abs(velocity.x) > rotationThreshold || abs(velocity.y) > rotationThreshold {
                                ...
                                timer = Timer.scheduledTimer(withTimeInterval: 0.01, repeats: true) { timer in
                                    ...
                                    if let snap = angleSnap {
                                        rotationAngle = snapToAngle(rotationAngle,
                                                                    snap: snap,
                                                                    velocity: lastVelocity,
                                                                    animation: { interpolationFactor in
    ...
    private func calculateRotationAngle(value: DragGesture.Value, geometry: GeometryProxy, angleSnap: Double?) -> Angle {
        ...
        let angleDifference = atan2(
            startVector.dy * endVector.dx - startVector.dx * endVector.dy,
